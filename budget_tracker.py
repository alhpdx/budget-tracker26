...
def update_notion(spending):
    ...
    if not NOTION_TOKEN or not DATABASE_ID:
        print("Notion credentials not set - skipping Notion update")
        print("Spending summary:")
        for category, amount in spending.items():
            budget = BUDGETS[category]
            remaining = budget - amount
            pct = (amount / budget * 100) if budget > 0 else 0
            print(f"  {category}: ${amount:.2f} / ${budget} ({pct:.0f}%)")
        return
    ...
    for page in pages:
        ...
        if category_name:
            ...
            if category and category in spending:
                ...
                update_data = {
                    "properties": {
                        "Spent": {"number": spent_amount},
                        "Remaining": {"number": max(0, budget_amount - spent_amount)},
                        "% Used": {"number": round((spent_amount / budget_amount) * 100) if budget_amount > 0 else 0}
                    }
                }
